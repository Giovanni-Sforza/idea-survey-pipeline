...
- `/skill:research-review` — paper review
- `/skill:auto-review-loop` — iterative review
- `/skill:paper-plan` — outline review
- `/skill:paper-write` — section review
- `/skill:paper-figure` — figure quality review
- `/skill:rebuttal` — stress test
- `/skill:meta-optimize` — patch review
- Any skill that sends artifacts to the `research-reviewer` subagent

## Multi-round review

Multi-round review within the SAME thread uses the Agent tool with `resume`:
...
Agent:
  ...
  prompt: |
    Please check whether the issues you raised in the previous round
    have been genuinely addressed in the updated files.
```

The reviewer may reference its own previous feedback to check resolution — but the executor must still not include executor interpretations of that feedback.
