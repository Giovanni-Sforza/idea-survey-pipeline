...
- **Reproducibility**: agent_id preservation allows conversation continuation
- **Meta-optimize input**: richer data for harness improvement analysis

## When to Trace

After **every** `Agent` tool call with `subagent_type: "research-reviewer"` (or other reviewer subagents) that serves a reviewer/critique function. This includes review scoring, experiment auditing, claim verification, idea critique, and patch gating.

Do NOT trace: purely informational LLM calls (e.g., code generation that is not a review).
...
After each reviewer Agent call, save the trace using the helper script:
...
bash tools/save_trace.sh \
  ...
  --purpose "<purpose>" \
  --model "<model>" \
  --agent-id "<agent_id from response>" \
...
If the `tools/save_trace.sh` script is not available, write the files directly following the schema below.
...
This allows `/skill:meta-optimize` to discover traces without reading the full trace files.
